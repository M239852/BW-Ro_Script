...
| key | meaning | default |
|---|---|---|
| `bobber_score_drop` | Template match score drop (vs cast-time rest score) that qualifies a frame as "dropped". Raise if ambient effects partially obscure the bobber, lower if real bites don't drop the score enough. | `0.35` |
| `bobber_pos_shift` | Manhattan pixel distance from the bobber's cast-time home at which a frame qualifies as "shifted". | `6` |
| `bobber_lost_ms` | **The most important knob.** Rolling-window size in milliseconds. A strike fires when **>=80% of the last `bobber_lost_ms` frames had BOTH a score crater AND a position shift** (AND, not OR). Rain/wind/ambient splashes create isolated dropped frames scattered among clean frames, so their rolling fraction stays well below 80%. A real bite pulls the bobber underwater continuously so the fraction hits ~100% within one window. Raise toward 800-1000 ms if weather still false-triggers; lower toward 400 ms if real bites are being rejected. | `600` |

> **Cast-settle behaviour.** The initial cast-impact splash on the water is large and can still be dissipating 2+ seconds after cast. The bot now uses an **adaptive settle**: after a minimum 0.8 s pause it polls the bobber region every 0.2 s and waits until 4 consecutive frames have a frame-to-frame delta below 4.0 (the water has actually stabilized), capped at 6 s maximum. This ensures the bobber template is captured from post-splash water — otherwise the splash-polluted baseline fires a false strike the instant the splash clears. You should see `[bot] settling t=X.Xs delta=... quiet=...` lines in `--debug` followed by `[bot] water settled after X.Xs` before the bot enters WAITING_SINK.
